...
all_tables = get_tables(open('../../data/unit 1/EN-FINAL Table 9.pdf', 'rb'))
...
for table in all_tables:
    for row in table[5:]:
        ...
        if len(headers) == len(row):  # Ensure equal length of headers and row
    ...
    if break_flag:  # Break the outer loop if Zimbabwe is found
        break
